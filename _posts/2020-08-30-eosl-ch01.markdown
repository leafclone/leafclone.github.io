...
& \text{We are just going to use without proof that the estimates are going to be unbiased}\\
...
No, since for this contrived example, $p$ also affects the _true_ function $y=x^T\beta$, and we use the same $p$ for true function as well as modeling. Note in our previous analysis, curse of dimensionality makes the error increase exponentially with $p$. With the rigid lienar model, it only increases linearly. 
